...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    -- FIXED: only count PAID entries for revenue
    'total_revenue',   (SELECT COALESCE(SUM(fe.paid_amount),0) FROM financial_entries fe WHERE fe.tenant_id=_tid AND fe.entry_type='revenue' AND fe.status::text IN ('paid','partial') AND fe.created_at BETWEEN _from AND _to),
    -- FIXED: only count PAID entries for expenses
    'total_expenses',  (SELECT COALESCE(SUM(fe.paid_amount),0) FROM financial_entries fe WHERE fe.tenant_id=_tid AND fe.entry_type='expense' AND fe.status::text IN ('paid','partial') AND fe.created_at BETWEEN _from AND _to),
    -- FIXED: only count PAID commissions
    'total_commissions',(SELECT COALESCE(SUM(fe.paid_amount),0) FROM financial_entries fe WHERE fe.tenant_id=_tid AND fe.entry_type='commission' AND fe.status::text IN ('paid','partial') AND fe.created_at BETWEEN _from AND _to),
    ...
    -- FIXED: today revenue only PAID
    'today_revenue',   (SELECT COALESCE(SUM(fe.paid_amount),0) FROM financial_entries fe WHERE fe.tenant_id=_tid AND fe.entry_type='revenue' AND fe.status::text IN ('paid','partial') AND fe.created_at BETWEEN _today_start AND _today_end),
    ...
    -- FIXED: avg ticket only PAID
    'avg_ticket_value', (SELECT ROUND(AVG(fe.paid_amount)::numeric,2) FROM financial_entries fe WHERE fe.tenant_id=_tid AND fe.entry_type='revenue' AND fe.status::text IN ('paid','partial') AND fe.created_at BETWEEN _from AND _to AND fe.service_order_id IS NOT NULL AND fe.paid_amount > 0),
  ...
  -- FIXED: collection point revenue only PAID
  ...
  _r := _r || jsonb_build_object('monthly_trend', (
    SELECT COALESCE(jsonb_agg(row_to_json(x) ORDER BY x.month), '[]'::jsonb) FROM (
      SELECT to_char(d, 'YYYY-MM') AS month,
        ...
        (SELECT COALESCE(SUM(fe2.paid_amount),0) FROM financial_entries fe2 WHERE fe2.tenant_id=_tid AND fe2.entry_type='revenue' AND fe2.status::text IN ('paid','partial') AND date_trunc('month',fe2.created_at)=d) AS revenue,
        (SELECT COALESCE(SUM(fe3.paid_amount),0) FROM financial_entries fe3 WHERE fe3.tenant_id=_tid AND fe3.entry_type='expense' AND fe3.status::text IN ('paid','partial') AND date_trunc('month',fe3.created_at)=d) AS expenses,
        0 AS profit
  ...
  -- FIXED: team ranking so_revenue only PAID
