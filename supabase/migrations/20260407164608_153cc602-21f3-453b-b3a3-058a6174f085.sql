...
-- Drop and recreate the search_service_orders function with advanced filters
...
BEGIN
  ...
  WHERE so.tenant_id = _tenant
    AND (_status IS NULL OR so.status = _status)
    AND (_priority IS NULL OR so.priority = _priority)
    ...
    -- Origin filter: 'counter' = no collection point, 'partner' = has collection point
    AND (
      _origin IS NULL
