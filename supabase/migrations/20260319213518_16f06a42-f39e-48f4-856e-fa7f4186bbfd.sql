  ...
  _r := _r || jsonb_build_object('collection_point_orders', (
    SELECT COALESCE(jsonb_agg(row_to_json(x)), '[]'::jsonb) FROM (
      SELECT so.collection_point_id AS cp_id, cp.name, COUNT(DISTINCT so.id) AS count,
        COALESCE((SELECT SUM(fe2.amount) FROM financial_entries fe2 WHERE fe2.service_order_id = ANY(ARRAY_AGG(so.id)) AND fe2.entry_type='revenue' AND fe2.status::text<>'cancelled' AND fe2.tenant_id=_tid),0) AS revenue,
