-- RPC: search service_orders including customer name and device info
...
BEGIN
  ...
  _tid := (SELECT (auth.jwt()->'app_metadata'->>'tenant_id')::uuid);
...
-- RPC: search financial_entries including customer, supplier, OS number
...
-- RPC: search sales including customer name
...
-- RPC: search receivables including customer name
...
-- RPC: search stock_movements including product name/sku
...
-- RPC: search warranties including customer, OS, device
...
-- RPC: search warranty_returns including warranty number, customer
