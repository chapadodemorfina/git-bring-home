CREATE OR REPLACE FUNCTION public.dashboard_summary(_from timestamp with time zone, _to timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
REVOKE EXECUTE ON FUNCTION public.dashboard_summary(timestamptz, timestamptz) FROM PUBLIC;
